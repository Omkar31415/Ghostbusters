...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Question 4 
        l = legal
        lg = livingGhostPositionDistributions
        pac = pacmanPosition
        ba = None
        cgp = None
        minf = float("inf")
        ...
        for dist in lg:
            minfa = float("inf")
            gopi = dist.argMax()
            distToGhost = self.distancer.getDistance(pac, gopi)
            if distToGhost < minf:
                minf = distToGhost
                cgp = gopi
        ...
        if cgp:
            for at in l:
                nexp = Actions.getSuccessor(pac, at)
                aa = self.distancer.getDistance(nexp, cgp)
                if aa < minf:
                    minf = aa
                    ba = at
        # RES
        # Question 4 (8 points): Exact Inference Full Test
        return ba
